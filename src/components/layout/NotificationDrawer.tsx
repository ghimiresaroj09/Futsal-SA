...
export function NotificationDrawer({ onClose, onViewAll, onViewBooking }: { onClose: () => void; onViewAll: () => void; onViewBooking: (redirectUrl: string) => void }) {
  ...
  return <><div className="notification-drawer-backdrop" onClick={onClose} /><aside className="notification-drawer"><div className="drawer-header"><div><p className="eyebrow">Booking activity</p><h2>Notifications</h2></div><button className="modal-close" onClick={onClose}><X size={18} /></button></div><div className="drawer-toolbar"><div className="notification-tabs">{(['ALL', 'UNREAD', 'READ'] as const).map((value) => <button key={value} className={filter === value ? 'selected' : ''} onClick={() => setFilter(value)} disabled={loading}>{value === 'ALL' ? 'All' : value === 'UNREAD' ? <>Unread <b>{unread}</b></> : 'Read'}</button>)}</div><button className="drawer-mark-all" disabled={!unread || loading} onClick={() => setItems((current) => current.map((item) => ({ ...item, is_read: true })))}><CheckCheck size={14} />Mark all read</button></div><div className="drawer-list">{loading ? <DrawerSkeleton /> : <>{visible.map((item) => <div className={`drawer-notice ${item.is_read ? '' : 'unread'}`} key={item.id}><div className={`notification-icon ${item.title.toLowerCase().includes('cancel') ? 'warning' : 'booking'}`}>{item.title.toLowerCase().includes('cancel') ? <CircleAlert size={16} /> : <Bell size={16} />}</div><div className="drawer-notice-content"><h3>{item.title}</h3><p>{item.message}</p><time>{item.time_ago}</time><button className="drawer-view-booking" onClick={() => onViewBooking(item.redirect_url)}>View booking →</button></div>{!item.is_read && <button className="drawer-read" onClick={() => void read(item.id).catch(() => {})}><Check size={14} /></button>}</div>)}{!visible.length && <div className="empty-notifications"><Bell size={22} /><p>No notifications here.</p></div>}</>}</div><button className="view-all-notifications" onClick={onViewAll}>View all notifications</button></aside></>
}
