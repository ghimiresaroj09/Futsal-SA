...
type Response = {
  success: boolean;
  message: string;
  data: { unread_count: number; results: Notice[] };
};
...
export function NotificationDrawer({
  ...
  onClose: () => void;
  onViewAll: () => void;
  onViewBooking: (redirectUrl: string) => void;
}) {
  ...
  const read = async (id: string) => {
    ...
    try {
      ...
      const body = await response.json().catch(() => ({}));
      if (!response.ok || !body.success)
        throw Error(body.message || "Unable to mark notification as read.");
      setItems((current) =>
        current.map((item) =>
          item.id === id
  ...
  return (
    <>
      ...
      <aside className="notification-drawer">
          ...
          <div className="notification-tabs">
            {(["ALL", "UNREAD", "READ"] as const).map((value) => (
              <button
                ...
                disabled={loading || Boolean(marking)}
              >
                {value === "ALL" ? (
                  "All"
                ) : value === "UNREAD" ? (
        ...
        <div className="drawer-list">
          {loading ? (
            ...
            <>
              {visible.map((item) => (
                <div
                  className={`drawer-notice ${item.is_read ? "" : "unread"}`}
                  key={item.id}
                >
                  <div
                  ...
                  <div className="drawer-notice-content">
                    <h3>{item.title}</h3>
                    <p>{item.message}</p>
                    <time>{item.time_ago}</time>
                    <button
                      className="drawer-view-booking"
                      onClick={() => onViewBooking(item.redirect_url)}
                    >
                      View booking →
                    </button>
                  </div>
                  {!item.is_read && (
                    <button
                      className="drawer-read"
                      disabled={Boolean(marking)}
                      onClick={() =>
                        void read(item.id).catch((error) =>
                          showToast(
                            error instanceof Error
                              ? error.message
        ...
        <button className="view-all-notifications" onClick={onViewAll}>
  ...
  return (
    ...
    >
      {Array.from({ length: 5 }, (_, index) => (
        <div className="drawer-notice notification-skeleton" key={index}>
