...
export function AppShell() {
  ...
  useEffect(() => {
    ...
    const handleUnreadCount = (event: Event) =>
      updateBadge((event as CustomEvent<number>).detail);
    ...
    const normalizePageSizeOptions = () => {
      document
        .querySelectorAll<HTMLSelectElement>(
          ".rows-select select, .booking-rows-select select",
        )
        .forEach((select) => {
          ...
          if (
            ...
            return;
          const selectedValue = expected.includes(select.value)
            ? select.value
            : "10";
          select.replaceChildren(
            ...expected.map(
              (value) =>
                new Option(value, value, false, value === selectedValue),
            ),
          );
        });
    };
    ...
    <div className="app-shell">
      ...
      >
        <div className="brand">
          ...
          <span className="brand-name">Nexus FMS</span>
          <button
        ...
        <nav className="main-nav">
            ...
            label === "Slot Management" ? (
              <div key={to} className="slot-nav-group">
                ...
                >
                  ...
                  <ChevronDown
                    size={15}
                    className={slotMenuOpen ? "slot-chevron-open" : ""}
                  />
                </button>
                {slotMenuOpen && (
          ...
          <button className="nav-item nav-logout" onClick={requestLogout}>
            <LogOut size={18} />
            <span>Logout</span>
          </button>
        </nav>
          ...
          <div className="topbar-actions">
            ...
            {notificationsOpen && (
              <NotificationDrawer
                ...
                onViewBooking={(redirectUrl) => {
                  setNotificationsOpen(false);
                  navigate(redirectUrl.replace(/^\/admin(?=\/|$)/, ""));
                }}
              />
            )}
      ...
      {logoutConfirmationOpen && (
        <div
          ...
          onClick={() => !logoutPending && setLogoutConfirmationOpen(false)}
        >
          <div
