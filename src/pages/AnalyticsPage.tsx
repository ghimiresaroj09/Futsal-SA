...
export function AnalyticsPage() {
  ...
  return <div className="analytics-page">
    ...
    <section className="analytics-detail-grid"><article className="analytics-panel"><div className="analytics-panel-heading"><div><h2>Revenue by payment method</h2><p>Revenue attributed to each payment channel</p></div><WalletCards size={19} className="panel-icon" /></div><div className="analytics-list">{analytics.revenue_by_payment_method.map((item) => <div className="analytics-list-row" key={item.method}><div><strong>{item.label}</strong><small>{item.booking_count} bookings · {item.percentage}%</small></div><strong>{money(item.revenue)}</strong></div>)}</div></article><article className="analytics-panel"><div className="analytics-panel-heading"><div><h2>Payment status</h2><p>Payment totals across all bookings</p></div><span className="payment-total">{analytics.payment_status.total} bookings</span></div><div className="payment-status-table"><div className="payment-status-head"><span>Status</span><span>Bookings</span><span>Amount</span></div>{analytics.payment_status.breakdown.map((item) => <div className="payment-status-row" key={item.status}><span className="payment-status-name"><i className={statusTone(item.status)} />{item.label}</span><span>{item.count}</span><strong>{money(item.amount)}</strong></div>)}</div></article></section>
