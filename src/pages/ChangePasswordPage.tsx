  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const response = await authFetch(
        `${apiBase}/api/v1/auth/change-password/`,
        {
          method: "POST",
      ...
      if (!response.ok || !body.success)
        throw new Error(
          body?.message || body?.detail || "Unable to change password.",
        );
      clearAuthTokens();
      ...
      <form className="password-card" onSubmit={submit}>
        ...
        <div className="password-fields">
          <PasswordField
            ...
            onChange={(value) => update("old", value)}
          />
          <PasswordField
            label="New Password"
            value={form.next}
        ...
        <div className="form-actions">
          ...
          <button type="submit" className="primary-button" disabled={saved}>
            {saved ? (
              "Updating..."
            ) : (
              <>
