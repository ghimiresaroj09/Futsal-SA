...
export function NotificationsPage() {
  ...
  const markRead = async (id: string) => {
    ...
    try {
      ...
      const body = await response.json().catch(() => ({}))
      if (!response.ok || !body.success) throw new Error(body?.message || 'Unable to mark notification as read.')
    ...
    } catch (error) { showToast(error instanceof Error ? error.message : 'Unable to mark notification as read.', 'error') } finally { setMarking(null) }
  }
  ...
  return <div className="notifications-page"><div className="page-heading"><div><p className="eyebrow">Notification center</p><h1>Notifications</h1></div><button className="secondary-button" disabled={!unread || loading || Boolean(marking)} onClick={() => void markAllRead()}><CheckCheck size={16} />{marking === 'all' ? 'Marking as read...' : 'Mark all as read'}</button></div><section className="notifications-card"><div className="notification-toolbar"><div className="notification-tabs">{(['ALL', 'UNREAD', 'READ'] as const).map((value) => <button key={value} className={filter === value ? 'selected' : ''} onClick={() => setFilter(value)} disabled={loading}>{value === 'ALL' ? `All ${items.length}` : value === 'UNREAD' ? `Unread ${unread}` : 'Read'}</button>)}</div><span className="notification-total">{loading ? 'Loading notifications' : `${visible.length} notifications`}</span></div><div className="notification-list">{loading ? <NotificationsSkeleton /> : <>{visible.map((item) => <article className={`notification-row ${item.is_read ? 'is-read' : 'is-unread'}`} key={item.id}><div className={`notification-icon ${item.title.toLowerCase().includes('cancel') ? 'warning' : 'booking'}`}>{item.title.toLowerCase().includes('cancel') ? <CircleAlert size={17} /> : <Bell size={17} />}</div><div className="notification-body"><h3>{item.title}</h3><p>{item.message}</p><time>{item.time_ago}</time><button className="view-booking-link" onClick={() => navigate(item.redirect_url.replace('/admin', ''))}>View booking →</button></div>{!item.is_read ? <button className="mark-read" disabled={Boolean(marking)} onClick={() => void markRead(item.id)}><Check size={16} /><span>{marking === item.id ? 'Marking...' : 'Mark read'}</span></button> : <span className="read-label">Read</span>}</article>)}{!visible.length && <div className="empty-notifications"><Bell size={24} /><p>No notifications in this filter.</p></div>}</>}</div></section></div>
}
