...
async function refreshAccessToken() {
  ...
  refreshRequest = (async () => {
    ...
    if (!response.ok || !body.success || !body.data?.access)
...
type PaginatedResponse<T> = {
  success: boolean;
  message?: string;
  data: { next?: string | null; results: T[] };
};
...
api.interceptors.request.use((config) => {
  if (config.method && config.method.toUpperCase() !== "GET")
    axiosButtonReleases.set(config, showButtonSpinner());
...
api.interceptors.response.use(
  (response) => {
    axiosButtonReleases.get(response.config)?.();
    return response;
  },
  async (error) => {
